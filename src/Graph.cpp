...
Graph::Graph(std::vector<std::string> expr){}

void Graph::push_edge(int first, int second, std::string mark) {
    Graph::Edge tmp;
    tmp.first=first;
    tmp.second=second;
    tmp.mark=mark;
...
void Graph::construction(std::vector<std::string> path){
    std::string mark_1="None",mark_2="None";
    std::pair<int,int> first_last;
    Edge tmp;
    num_states=0;
    ...
    for(int i=0;i<ans.size();i++){
        std::cout<<ans[i].first<<" "<<ans[i].second<<" "<<ans[i].mark<<std::endl;
        print(ans);
    }

}
void Graph::print(std::vector<Graph::Edge> path) {
    std::ofstream out;
    out.open("Graph.gv");
    out<<"digraph automate {"<<std::endl;
    out<<"rankdir=LR;"<<std::endl;
    out<<"size=\"8,5\";"<<std::endl;
    for(int i=0;i<path.size();i++){
        out<<"\""<<path[i].first<<"\""<<"->"<<"\""<<path[i].second<<"\""<<"[label = \""<<path[i].mark<<"\"];"<<std::endl;
    }
    out<<"}"<<std::endl;
    out.close();
}
